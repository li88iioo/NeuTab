 ...
 * 3. 运行后会生成 `packages/ui/src/assets/site-icons/*.svg` 以及 `packages/ui/src/utils/iconUrlMap.ts`。
 * 4. 运行时通过 `url:` 导入按需加载这些小体积的静态资源。
 * 5. 支持 `pack: "custom"` 类型，用于自定义 SVG 图标（已预先放置在 packages/ui/src/assets/site-icons 目录）。
 */
...
const catalogPath = path.join(repoRoot, "packages", "ui", "src", "utils", "iconCatalog.json")
const titleCatalogPath = path.join(repoRoot, "packages", "ui", "src", "utils", "iconTitleCatalog.json")

const outDir = path.join(repoRoot, "packages", "ui", "src", "assets", "site-icons")
const outMapPath = path.join(repoRoot, "packages", "ui", "src", "utils", "iconUrlMap.ts")
...
const main = () => {
	...
	for (const a of sortedAssets) {
		lines.push(`const ${a.varName} = new URL(\"../assets/site-icons/${a.fileName}\", import.meta.url).toString()`)
	}
	lines.push("")
