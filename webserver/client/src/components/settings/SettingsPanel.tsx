...
const localExtStorage = new Storage({ area: "local" }) // local storage for groups
...
const SettingsPanel = ({ onClose }: SettingsPanelProps) => {
   ...
   * @description
   * 1. 汇总所有核心设置。
   * 2. 从服务器图标 API 获取图标并编码为 Base64，实现全量备份。
   */
  const buildBackupPayload = async () => {
    const baseSettings = await storage.getMany(Object.keys(DEFAULT_SETTINGS)) as Record<string, any>
    ...
    const settings = {
      ...
      searchEngines,
      currentEngine,
      themeMode: (baseSettings.themeMode as ThemeMode | undefined) || themeMode || DEFAULT_SETTINGS.themeMode,
      visualTheme: (baseSettings.visualTheme as VisualTheme | undefined) || visualTheme || DEFAULT_SETTINGS.visualTheme,
      language: (baseSettings.language as Language | undefined) || language || DEFAULT_SETTINGS.language,
      quickLaunchGroups: safeGroups,
      // Clamp layout to supported range on export (backward compatible).
      contentMaxWidth: clampNumber(
        Number(baseSettings.contentMaxWidth ?? contentMaxWidth ?? DEFAULT_SETTINGS.contentMaxWidth),
        LAYOUT_LIMITS.maxWidth.min,
        LAYOUT_LIMITS.maxWidth.max
      ),
      contentPaddingX: clampNumber(
        Number(baseSettings.contentPaddingX ?? contentPaddingX ?? DEFAULT_SETTINGS.contentPaddingX),
        LAYOUT_LIMITS.paddingX.min,
        LAYOUT_LIMITS.paddingX.max
      ),
      contentPaddingTop: clampNumber(
        Number(baseSettings.contentPaddingTop ?? contentPaddingTop ?? DEFAULT_SETTINGS.contentPaddingTop),
        LAYOUT_LIMITS.paddingTop.min,
        LAYOUT_LIMITS.paddingTop.max
      ),
      contentPaddingBottom: clampNumber(
        Number(baseSettings.contentPaddingBottom ?? contentPaddingBottom ?? DEFAULT_SETTINGS.contentPaddingBottom),
        LAYOUT_LIMITS.paddingBottom.min,
        LAYOUT_LIMITS.paddingBottom.max
      ),
      iconBorderRadius: clampNumber(
        Number(baseSettings.iconBorderRadius ?? iconBorderRadius ?? DEFAULT_SETTINGS.iconBorderRadius),
        LAYOUT_LIMITS.iconBorderRadius.min,
        LAYOUT_LIMITS.iconBorderRadius.max
      ),
      cardSize: clampNumber(
        Number(baseSettings.cardSize ?? cardSize ?? DEFAULT_SETTINGS.cardSize),
  ...
  const handleImport = async (file: File) => {
    try {
      ...
      const { updates, groups: groupsData, customIcons, meta } = normalizeBackupImport(rawData, language || DEFAULT_SETTINGS.language)

      // 保存常规设置至 sync storage（批量写入更快）
      ...
      if (customIcons) {
        ...
        for (const [appId, base64] of Object.entries(customIcons)) {
          try {
            ...
            if (!mimeType.startsWith("image/")) throw new Error("Invalid image type")
      ...
      try {
        ...
        const lang = (updates.language as string | undefined) ?? (meta.language as string | undefined) ?? (language as string | undefined)
          ...
          <div className="settings-content">
            {activeSection === "personalization" && (
              <section className="settings-section" key="personalization">
                ...
                {/* 视觉主题岛屿 */}
                <div className={`settings-island${isVisualThemeOpen ? ' dropdown-open' : ''}`}>
