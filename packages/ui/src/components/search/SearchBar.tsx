...
interface SearchResult {
  ...
  type: "app" | "bookmark"
  /** 对应的数据对象 */
  data: QuickLaunchApp | BookmarkNode
}
...
const SearchBarInner = () => {
        ...
        const runSearch = () => {
          chromeApi.bookmarks.search(query, (results: any[]) => {
            ...
            const bookmarkMatches: SearchResult[] = results
              ...
              .slice(0, 50)
              .map((b) => ({ type: "bookmark", data: { id: String(b.id), title: String(b.title ?? ""), url: String(b.url || "") } }))
  ...
  const suggestions = useMemo(() => {
    ...
    return searchResults.map((item) => {
      ...
      const bookmarkData = data as BookmarkNode
      const url = isApp
        ? ((data as QuickLaunchApp).url || (data as QuickLaunchApp).internalUrl || "")
        : (bookmarkData.url || "")
      const name = isApp ? (data as QuickLaunchApp).name : bookmarkData.title
      const id = isApp ? (data as QuickLaunchApp).id : bookmarkData.id
