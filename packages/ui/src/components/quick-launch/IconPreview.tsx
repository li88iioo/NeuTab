...
const IconPreview = ({
    ...
    const [debouncedFaviconUrl, setDebouncedFaviconUrl] = useState<string | null>(null)
    ...
    // 防抖获取 favicon URL，避免输入过程中频繁请求
    useEffect(() => {
        if (debounceTimer.current) {
            clearTimeout(debounceTimer.current)
        }
        debounceTimer.current = setTimeout(() => {
            setDebouncedFaviconUrl(getBrowserFaviconUrl(url, 64))
        }, FAVICON_DEBOUNCE_MS)
    ...
    const candidates = useMemo(() => {
        ...
        return [localIcon, customIcon, builtInIconUrl, debouncedFaviconUrl].filter(Boolean) as string[]
    }, [localIcon, customIcon, builtInIconUrl, debouncedFaviconUrl])

    useEffect(() => {
        setSrcIndex(0)
    }, [name, url, customIcon, localIcon, builtInIconUrl, debouncedFaviconUrl, iconStyle])
